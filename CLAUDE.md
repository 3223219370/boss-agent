...
13. **分析记录历史**（`src/services/history/`）：chrome.storage.local **单键存数组**（键 `analysisHistory`，最新在前）；该键**不进 `ALL_STORAGE_KEYS`**（getAppConfig 全量读取会污染配置——该数组已显式化，**新增配置键必须同步加入**；服务配置预设键 `llmPresets` 同为独立存储键，不进此数组，见 src/services/storage）。写入模式 = `analyzeWithLlm` 立即写初始记录（仅 `result.ok` 入库，失败静默不阻塞分析）+ 打招呼决策点 `finalizeGreet` 补写 `GreetOutcome`（sent 自动发送成功 / failed 自动发送失败降级 / manual 手动模式未代发 / none 不匹配）；补写幂等守卫：仅 match 且当前为 none 时生效（防 failed 被 manual 覆盖）；上限 `HISTORY_LIMIT=500`（`src/constant/history.ts`）超限自动淘汰最旧；popup 侧 `useAnalysisHistory` hook 读取 + `onChanged` 实时刷新
